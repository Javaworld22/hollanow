...
//    public static final String CATEGORY = "CATEGORY";
  ...
  private void getUsersByIndustry(String industry) {
    Call<List<User>> call = hollaNowApiInterface.getUserByIndustry(mSharedPref.getToken(), industry, "Nigeria"); // Nigeria
      ...
      public void onResponse(Call<List<User>> call, Response<List<User>> response) {
        if (response.code() == 200) {
            for(int i = 0; i < response.body().size();i++) {
                ...
                Log.e(TAG, "print from get user by name: "+user.getName());
               if(mSharedPref.getCountry().equals(user.getCountry()))
                   user1.add(user);
                Log.e(TAG, "print from get user by country: "+user.getCountry());
            }
          ...
          mAdapter = new UserAdapter(user1, CategoryActivity.this);

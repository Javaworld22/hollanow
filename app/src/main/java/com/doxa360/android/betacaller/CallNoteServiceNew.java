 ...
 */
public class CallNoteServiceNew extends Service implements  View.OnAttachStateChangeListener{
...
//    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (check_for_dataString == null && check_for_dataInt == 0)
            ...
            String noteJSon = intent.getExtras().getString("callnote");
        ...
        content.setOnClickListener(new View.OnClickListener() {
           // @TargetApi(Build.VERSION_CODES.JELLY_BEAN)
            ...
            public void onClick(View v) {
                ...
                try {
                    if(mSharePref != null)
                        mSharePref.setOverlayAppearance(0);
                    onDestroy();
                ...
                //   intent.putExtra(BetaCaller.USER_PROFILE, callNoteUser);  //////////////////////////
...
//                }
        ...
        if (intent != null) {
            ...
            if (intent.getExtras() != null)
                if (intent.getExtras().getString("callnote_service") != null) {
                    if (intent.getExtras().getString("callnote_service").equals("Just show callnote notification")) {
                        if (cNote != null) {
                        ...
                        if (chatheadView != null) {
                            if(chatheadView.getParent() == null) {
                                windowManager.addView(chatheadView, param);
                                Log.e(TAG, "Error at 3 ");
                            }
                            else {
                                windowManager.updateViewLayout(chatheadView, param);
                                Log.e(TAG, "Error at 4 ");
                            }
                        }
                    }
                } else {
                    if (chatheadView != null) {
                        ...
                        int overlay = mSharePref.getOverlayAppearance();
                        Log.e(TAG, "Number of Times by Overlay "+overlay);
                        if(!chatheadView.isShown())
                            windowManager.addView(chatheadView, param);
                        else
                            windowManager.updateViewLayout(chatheadView, param);
                           // WindowManager.LayoutParams paramName =
                           ...
                           // windowManager.addView(chatheadView, params);
                            ...
                            overlay++;
                            mSharePref.setOverlayAppearance(overlay);
                            Log.e(TAG, "Error at 5 ");


                        chatheadView.addOnAttachStateChangeListener(this);
        ...
        if (chatheadView != null) {
            if(chatheadView.getParent() == null) {
                windowManager.addView(chatheadView, param);
                Log.e(TAG, "Error at 7 ");
            }
            else {
                windowManager.updateViewLayout(chatheadView, param);
                Log.e(TAG, "Error at 8 ");
            }
        }
    }
    ...
    public void onViewAttachedToWindow(View v){
        Log.e(TAG, "View is now attached");
       // windowManager.updateViewLayout(chatheadView, params);
    }
    @Override
    public void onViewDetachedFromWindow(View v){
        Log.e(TAG, "View is now detached");
   ...
   // @TargetApi(Build.VERSION_CODES.HONEYCOMB)
    ...
    private void sendNotification(List<CallNote> callNote){
        ...
        for(int i = 0; i<callNote.size();i++) {
            ...
            Notification.Builder mNotification1 = new Notification.Builder(getApplicationContext())
    ...
    private void searchNumber(CharSequence charSequence) {

        //windowManager.addView(mProgressDialog.getCurrentFocus(),null);

        isFinished = false;
       //final User user;
        Log.e(TAG, "Print Usernames here :");
        Log.e(TAG, "Print Usernames here :");
       // Log.e(TAG, "Print Usernames here :");
        ...
        call.enqueue(new Callback<List<User>>() {
            ...
            public void onResponse(Call<List<User>> call, Response<List<User>> response) {
                if (response.code() == 200) {
                    if(response.body().size() > 0)
                       callNoteUser = response.body().get(0);
                    isFinished = true;
                    for(int i=0;i<response.body().size();i++) {
                        Log.e(TAG, "Response from searchNumber " + " " + response.body().get(i).toString());
                      //  Log.e(TAG, "Response from searchNumber " + " " + response.body().get(i).toString());
                      //  Log.e(TAG, "Response from searchNumber " + " " + response.body().get(i).toString());
                    }
            ...
            public void onFailure(Call<List<User>> call, Throwable t) {
                ...
                isFinished = true;
                Log.e(TAG, "Error occurred here  " + t.getMessage() );
                //Toast.makeText(mContext, "Network error. Try again", Toast.LENGTH_SHORT).show();
               // mProgressDialog.dismiss();
            }
        });

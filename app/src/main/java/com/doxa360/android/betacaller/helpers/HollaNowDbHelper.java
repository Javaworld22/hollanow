...
public class HollaNowDbHelper extends SQLiteOpenHelper {
    ...
    public static final String TABLE_HOLLA_CONTACTS = "HollaContacts";
    private static final String COLMN_CONTACT_IMAGE_NAME = "image_name";
    private static final String COLMN_CONTACT_IMAGE_DATA = "image_data";
    private static final String COLUMN_CONTACT_LAST_UPDATE = "last_update";
    private static final String COLUMN_CONTACT_CREATED_DATE = "created_date";
    ...
    private static final String SQL_CACHE_HOLLA_CONTACTS =
            "CREATE TABLE IF NOT EXISTS " + TABLE_HOLLA_CONTACTS + " (" +
                    ...
                    COLUMN_CONTACT_LAT + " REAL," +
                    COLUMN_CONTACT_LONG + " REAL," +
                    COLMN_CONTACT_IMAGE_NAME + " TEXT," +
                    COLMN_CONTACT_IMAGE_DATA + " BLOB," +
                    COLUMN_CONTACT_LAST_UPDATE + " TEXT," +
                    COLUMN_CONTACT_CREATED_DATE + " TEXT," +
                    COLUMN_NULLABLE + " TEXT," +
                    " UNIQUE (" + COLUMN_CONTACT_ID +") ON CONFLICT REPLACE" +
                    " )";
...
//                    " )";
    ...
    private static final String SQL_DELETE_TABLE_HOLLA_CONTACTS =
            "DROP TABLE IF EXISTS " + TABLE_HOLLA_CONTACTS;
...
//        super(context, name, factory, version, errorHandler);
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
       ...
       // sqLiteDatabase.execSQL(SQL_CACHE_CONTACTS);
        sqLiteDatabase.execSQL(SQL_CACHE_HOLLA_CONTACTS);
    ...
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int i, int i1) {
        ...
        sqLiteDatabase.execSQL(SQL_DELETE_TABLE_HOLLA_CONTACTS);
    ...
    public void clearCallLogs() {
        ...
        try {
            ...
            try {
                sqLiteDatabase.wait(1000);
            }catch (InterruptedException e){
                Log.e(TAG, "sqliteDatabase wait is interrupted");
            }
        }
        sqLiteDatabase.close();
    ...
    public synchronized long cacheHollaContacts(Parse_Contact contact) {
...
//        Log.e("DB", "im here o");
       ...
       // String thumbnail = contact.getThumbnailUrl();
        ...
        String imageUrl = contact.getThumbnailUrl();
        Bitmap imageData = contact.getThumbnail();

        //Todo: get byte Array from Bitmap
        byte[] imageByte = null;
   try {
        imageByte = getByteArray(imageData);
   }catch (NullPointerException a){
       Log.e(TAG, "Exception Occured "+a.getMessage());
   }
        ...
        values.put(HollaNowDbHelper.COLMN_CONTACT_IMAGE_NAME, imageUrl);
        values.put(HollaNowDbHelper.COLMN_CONTACT_IMAGE_DATA, imageByte);
       // values.put(HollaNowDbHelper.COLUMN_CONTACT_TYPE, accountType);

        if(db.isOpen()) {
            try {
                newRowId = db.insert(
                        HollaNowDbHelper.TABLE_HOLLA_CONTACTS,
...
//        Log.e("CACHE CONTACT NEW:", newRowId+"");
    ...
    public synchronized List<bimps> allNotification() {
        ...
        mCursor.close();
        close();
        Log.e(TAG, "Check out notification here "+notificationtList.toString());
        return notificationtList;
    }

    public synchronized List<Parse_Contact> allHollaContacts() {
        ...
        String[] projection = {
                ...
                COLUMN_CONTACT_LAT,
                COLUMN_CONTACT_LONG,
                COLMN_CONTACT_IMAGE_NAME,
                COLMN_CONTACT_IMAGE_DATA,
                COLUMN_CONTACT_LAST_UPDATE,
                COLUMN_CONTACT_CREATED_DATE
...
//                }
                        ...
                        contact.setThumbnailUrl(mCursor.getString(mCursor.getColumnIndexOrThrow(COLMN_CONTACT_IMAGE_NAME)));
                        byte[] image = mCursor.getBlob(6);

                        contact.setThumbnail(getImage(image));
                        contact.setLastSeen(mCursor.getString(mCursor.getColumnIndexOrThrow(COLUMN_CONTACT_LAST_UPDATE)));
                        contact.setCreatedDate(mCursor.getString(mCursor.getColumnIndexOrThrow(COLUMN_CONTACT_CREATED_DATE)));
...
//            phoneNumber = phoneNumber.substring(1);
   ...
   if(phone != null) {
       ...
       Cursor mCursor = null;
      try {
           mCursor = db.rawQuery("select * from " + TABLE_CONTACTS + " where "
                  + COLUMN_CONTACT_PHONE + " ='" + phone + "'", null);
          Log.e(TAG, "Cursor mCursor ");
      }catch (SQLiteDatabaseLockedException ex){Log.e(TAG, "SQLiteDatabaseLockedException excption ");}
      catch (IllegalStateException ex){
          Log.e(TAG, "IllegalStateException excption ");
      }
...
//                Log.e(TAG, "DB: searching");
    ...
    public Parse_Contact searchContactByPhoneNumber(String search) {
        ...
        Log.e(TAG, "Query from SearchContactPhoneNumber "+query);
        Log.e(TAG, "Query from SearchContactPhoneNumber "+query);
        Log.e(TAG, "Query from SearchContactPhoneNumber "+query);
        Log.e(TAG, "Query from SearchContactPhoneNumber "+query);
        Log.e(TAG, "Query from SearchContactPhoneNumber "+query);
...
//    db.update("YOUR_TABLE", newValues, "name=? OR name=?", args);
    ...
    public static byte[] getByteArray(Bitmap bitmap){
        ...
        try {
            ...
            Log.e(TAG, "Exception occured here " + a.getMessage());
        }
        return  b;
    }
